...
const int HASH_TABLE_SIZE = 2179; //9971; //A prime roughly 10% larger
...
//
int CompletedEdges[ HASH_TABLE_SIZE];
int Node::Count = 1;
Node Nodes[ MAX_LENGTH * 2 ];
int size;
int string_size;
int required[100];
int temp_document_count;
int full_document_count;
int temp_hashtable_size;
...
void search_by_entity(int doc_id);
void store_eligible_entities();
void insert_document(Suffix active, char string_input[ MAX_LENGTH ], int docId);
void show(int current_n);
void insert_document_api(char string_input[ MAX_LENGTH ], int docId);
    ...
    for (int j = 0; j < HASH_TABLE_SIZE; j++) {
        ...
        if (s->is_changed == 0) {
            ...
            cout << setw(5) << s->start_node << " "
             ...
             cout<<"OK LAST CHARACTER IS "
                <<T[s->last_char_index];
            if (T[s->last_char_index]=='$') {
                s->last_char_index--;
                cout<<"OK LAST CHARACTER IS NOW"
                        <<T[s->last_char_index]
                        <<"\n";
            }
            TreeEntities[s->end_node ].sequence = "";
            TreeEntities[s->end_node ].end_node = s->end_node;
            cout << "before sequence of node"
                    << s->end_node
                    << "is "
                    << TreeEntities[s->end_node ].sequence
                    << " and then";
...
void show(int current_n) {
    ...
    cout << " Start  End  Suf  First Last  String   depth isLeaf terminationLabel docID\n";
    ...
    for (int j = 0; j < HASH_TABLE_SIZE; j++) {
        ...
        cout << setw(10) << Nodes[s->end_node].isLeaf << "--> ";
        cout << setw(10) << Nodes[s->end_node].termination_label << "  ";
        cout << setw(10) << EligibleEntities[s->end_node].document_id << "  ";
        cout << "\n";
        cout << TreeEntities[s->end_node ].end_node
                << ","
                << TreeEntities[s->end_node ].depth
...
insert_document(active, string_input, 1);
}
...
std::list<TreeEntity> eligible_entity_list;
...
eligible_entity_list.clear();
    for (int m = 0; m < 1000000
            ; m++) {
        if (EligibleEntities[m].document_id==doc_id) {

            eligible_entity_list.push_back(EligibleEntities[m]);
            cout << ",,,,,,,,,,,,,,,,,,"
                    << EligibleEntities[m].termination_char;
    ...
    eligible_entity_list.sort();
    TreeEntity entity = eligible_entity_list.back();
    char resultString[MAX_LENGTH];
    cout << "Here is the VALUE :::::"
            << entity.depth
            << " "
    ...
    cout << "Here are the strings :";
    k = entity.end_node;
    next =EligibleEntities[k];

    cout << k
            << next.sequence;
    while (k != 0) {

        k = EligibleEntities[k].parent;
        next = EligibleEntities[k];
...
void store_eligible_entities(int docId) {
    ...
    for (std::list<char>::iterator it = terminators.begin(); it != terminators.end(); it++) {
        ...
        for (int m = 0; m < 1000000
            ...
            if (Nodes[TreeEntities[m].end_node].isLeaf == 1 && Nodes[TreeEntities[m].end_node].termination_label == *it) {
                Nodes[TreeEntities[m].end_node].termination_label='%';
                entity_list.push_back(TreeEntities[m]);
                cout << ",,,,,,,,,,,,,,,,,,"
                        << TreeEntities[m].termination_char;
        ...
        // entity_list.sort();
        entity_list.sort();
        TreeEntity entity = entity_list.back(); //This is the most important place
        ...
        EligibleEntities[k].document_id=docId;
        cout << k
                << next.sequence;
        while (k != 0) {
...
void insert_document(Suffix active, char string_input[ MAX_LENGTH ], int docId) {
    ...
    store(active, string_input);
    terminators.push_back('$');
    dump_edges(N);
    store_eligible_entities(docId);
     terminators.clear();
...
int main() {
    ...
    // Open the named pipe
    HANDLE pipe = CreateFile( lpszPipename, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
 
    if (pipe == NULL || pipe == INVALID_HANDLE_VALUE) {
        wcout << "Failed to connect to pipe." <<  GetLastError() << endl;
        return 1;
    }
    ...
    BOOL result = ReadFile( pipe, buffer, 511 * sizeof(wchar_t*), &numBytesRead, NULL);
	
    if (result) {
        buffer[numBytesRead / sizeof(wchar_t)] =(wchar_t*) ""; // null terminate the string
        wcout << "Number of bytes read: " << numBytesRead << endl;
		t = (char*)buffer;
        wcout << "Message: " << t<< endl;
    } else {
        wcout << "Failed to read data from the pipe." << GetLastError() << endl;
    }
	...
	if(a != 0){
		cout << "Error while setting positioning indicator." << endl;
		return 1;
	}
	len = ftell(fp);
